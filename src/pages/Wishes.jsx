...
import { downloadWishStatus, downloadWishCollagePages } from '../utils/wishDownload';
...
const Wishes = () => {
  ...
  const handleExportAll = async () => {
    ...
    setError('');
    try {
      await downloadWishCollagePages(wishes, { heading: 'Birthday Wishes Collage' });
    } catch {
      setError('Could not export the wish collage. Please try again.');
    } finally {
      setExporting(false);
    }
  };
  ...
  return (
    <main className="section wishes-page">
      ...
      {user && loading ? (
        ...
        <>
          <div className="card wish-export-card">
            <div className="wish-export-actions">
              <button className="btn" type="button" onClick={handleExportAll} disabled={exporting}>
                {exporting ? 'Exporting wishes…' : 'Export all wishes'}
              </button>
              <p className="small-text">Download a multi-page collage of all wishes, grouped to keep 4–8 cards per page.</p>
            </div>
          </div>
